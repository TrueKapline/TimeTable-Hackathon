...
//require "time.php"; // ВРЕМЕННО. УДАЛИТЬ
//require "real_time_schedule.php"; // ВРЕМЕННО. УДАЛИТЬ
...
function test($date, $day_count) {
  ...
  while($day_count > 0) {
    ...
    for ($i = 0; $i < count($table_2); $i++) {
      ...
      /* Для удаления пары */
      elseif($table_2[$i]->type === 3) {
        $s_id = search_id($table_3, $table_2[$i]->affected_schedule_id);

        if($s_id != -1) {
          unset($table_3[$s_id]);
          $table_3 = array_values($table_3);
        }
      } 
      /* Для добавления пары */
      elseif($table_2[$i]->type === 4) {
  ...
  file_put_contents("../test.json", json_encode($table_3, JSON_UNESCAPED_UNICODE));
}
